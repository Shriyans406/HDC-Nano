...
export function ClassPredictionCard({ packet }) {
    ...
    return (
        <div className="bg-hdc-card border border-hdc-border rounded-xl p-6 shadow-lg flex flex-col justify-between h-56 relative overflow-hidden">
            <div
                className={`absolute -right-10 -bottom-10 w-40 h-40 rounded-full blur-3xl opacity-20 pointer-events-none ${isHighConfidence ? 'bg-hdc-cyan' : 'bg-hdc-amber'
                    }`}
            />
                ...
                <div className="flex items-center gap-1.5 mt-3 text-xs">
                    {isHighConfidence ? (
                        <>
                            ...
                            <span className="text-hdc-green font-medium">Strong Item Memory Lock</span>
                        </>
                    ) : (
                        <>
                            <AlertCircle className="w-3.5 h-3.5 text-hdc-amber" />
                            <span className="text-hdc-amber font-medium">Marginal Confidence</span>
                        </>
                    )}
                </div>
            </div>
